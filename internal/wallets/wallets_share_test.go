...
func TestSplitWallet(t *testing.T) {
	...
	for _, split := range splits {
		assert.Equal(t, split.Share.Threshold, threshold)
		assert.Equal(t, split.Share.NumShares, numShares)
	}
}
...
func TestJointWallet(t *testing.T) {
	...
	_, err = JointWallet(splits, wallet.Address, threshold)
	assert.Error(t, err)
}
